...
export const Tabs: FC<TabsProps> = (props) => {
  ...
  const navClass = classNames('cola-tabs-nav', {
    'nav-line': type === 'line',
    'nav-card': type === 'card',
  })
  const renderNavLinks = () => {
    return React.Children.map(children, (child, index) => {
      ...
      const classes = classNames('cola-tabs-nav-item', {
  ...
  return (
    <div className={`cola-tabs ${className}`}>
      <ul className={navClass}>{renderNavLinks()}</ul>
      <div className="cola-tabs-content">{renderContent()}</div>
    </div>
  )
}
