 ...
 *
 * ~~~js
 * import { Select } from 'cola-design'
...
export const Select: FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('cola-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="cola-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition in={menuOpen} animation="zoom-in-top" timeout={300}>
          <ul className="cola-select-dropdown">{generateOptions()}</ul>
        </Transition>
      </SelectContext.Provider>
      {multiple && (
        <div className="cola-selected-tags" style={{ maxWidth: containerWidth.current - 32 }}>
          {selectedValues.map((value, index) => {
            return (
              <span className="cola-tag" key={`tag-${index}`}>
                {value}
                <Icon
                  icon="times"
                  onClick={() => {
